...
func (lq *LeadQuotes) GetQuotesByLead(leadId string) error {
	sql := `
	SELECT q.id, q.budget, a.street_address_line1, a.street_address_line2, a.street_address_line3, ser.service, ser.id AS service_id,
	c.city, c.id AS city_id, s.state, s.id AS state_id, q.zip_code, q.created_at, ctry.country, ctry.id AS country_id,
	string_agg(qp.image_url, ',') AS photos
	FROM quote AS q
	LEFT JOIN service AS ser
	on ser.id = q.service_id
	LEFT JOIN quote_photo AS qp 
	on qp.quote_id = q.id
	...
	c.city, c.id, s.id, s.state, q.id, q.zip_code, q.created_at, q.budget, ctry.country, ctry.id;
	`
